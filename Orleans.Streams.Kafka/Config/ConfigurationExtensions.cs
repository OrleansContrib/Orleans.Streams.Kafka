...
{
	private const int _defaultCacheSize = 4096;

	public static KafkaStreamClientBuilder AddKafka(this IClientBuilder builder, string providerName) =>
		new(builder, providerName);

	public static KafkaStreamSiloBuilder AddKafka(this ISiloBuilder builder, string providerName) =>
		new(builder, providerName);

	public static KafkaStreamSiloHostBuilder AddKafka(this ISiloHostBuilder builder, string providerName) =>
		new(builder, providerName);
	...
	)
	{
		builder
			.ConfigureApplicationParts(parts =>
				parts.AddFrameworkPart(typeof(KafkaAdapterFactory).Assembly).WithReferences())
			...
			.Configure<SimpleQueueCacheOptions>(ob => ob.CacheSize = _defaultCacheSize);
	...
	)
	{
		builder
			.ConfigureApplicationParts(parts =>
				parts.AddApplicationPart(typeof(KafkaAdapterFactory).Assembly).WithReferences())
			.ConfigureServices(services =>
			{
				services
					.ConfigureNamedOptionForLogging<KafkaStreamOptions>(providerName)
			...
			.AddPersistentStreams(providerName, KafkaAdapterFactory.Create,
				stream => stream.Configure(configureOptions))
			.Configure<SimpleQueueCacheOptions>(options => options.CacheSize = _defaultCacheSize);
	...
	private static void AddJson(this IServiceCollection services, string providerName)
		=> services.AddSingletonNamedService<IExternalStreamDeserializer, JsonExternalStreamDeserializer>(providerName);
}